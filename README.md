...
1. По простому целочисленному id.  Я не стал делать логику решения проблем
   идентификации, которые мог бы тут придумать.  Если бы было больше времени
   и/или желания, можно было бы сделать отдельный сервис, который бы помогал
   переводить набор идентификационных признаков в ссылку на запись в бд.
...
Вообще тут бы что-то вроде очереди pending заявок, для которых бэк пытается
